...
The brain never opens X itself. check_steps() produces the exact
...
def readiness():
    return True, "x adapter ready"


def check_steps(account, threads):
    """Browser steps for the host agent to read the account's X DMs."""
    ...
    steps = [
        ...
        ("Login check FIRST: if you land on a login / sign-in / checkpoint "
         ...
         "user's approval."),
        ("If X shows the encrypted-messages passcode prompt, STOP and report "
         "it. Never type or ask for the passcode inside this flow; the "
         "account owner enters it themselves."),
        "Open the DM inbox and read the conversation list.",
    ]
    if known:
        steps.append(
