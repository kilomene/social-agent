...
Run: 2026-09-25 15:11 UTC — all exams execute the real CLI against isolated state dirs.
...
## exam-68: DM agent lifecycle: list/start/status/stop via CLI
Score: **6/6** [PASS]
- ✓ dm-agent list shows four platforms
- ✓ tiktok adapter reports not-ready
- ✓ dm-agent start x ok
- ✓ status shows active + poll floor
- ✓ dm-agent stop records reason
- ✓ status shows inactive + reason

## exam-69: dm-agent tick issues dm_check; trigger jumps queue; no pile-up
Score: **5/5** [PASS]
- ✓ plain tick skipped before poll interval
- ✓ triggered tick jumps the queue
- ✓ dm_check ticket issued with guard receipts
- ✓ dm_check steps are read-only
- ✓ trigger still respects the pile-up guard

## exam-70: dm-agent report ingests messages, advances cursor, fulfills ticket
Score: **5/6** [FAIL]
- ✓ report ok, one thread processed
- ✗ m1 already answered in-thread: only m3 is new — 2
- ✓ X automated_dms prohibited: refusal surfaces, nothing queued
- ✓ dm_check ticket fulfilled with observed evidence
- ✓ cursor advanced to m3 and pending cleared
- ✓ duplicate report is a no-op

## exam-71: TikTok adapter refuses; style gate + media fallback hold
Score: **4/4** [PASS]
- ✓ tiktok tick refuses with documented reason
- ✓ style gate splits em dash + drops banned phrase
- ✓ voice request gets honest text fallback, never silence
- ✓ credential bait is high-risk and never echoes secrets

## exam-72: message watcher retired from DM platforms; dm_agents owns DMs
Score: **4/4** [PASS]
- ✓ x registers 13 watchers (message retired)
- ✓ x:message is not registered
- ✓ tiktok:message is not registered
- ✓ youtube:message still registered (not a DM-agent platform)

---

# social-agent exam results

Run: 2026-09-25 15:12 UTC — all exams execute the real CLI against isolated state dirs.
...
Score: **6/6** [PASS]
- ✓ report ok, one thread processed
- ✓ m1 already answered in-thread: no draft for it, only m3 considered
- ✓ X automated_dms prohibited: refusal surfaces, nothing queued
- ✓ dm_check ticket fulfilled with observed evidence
- ✓ cursor advanced to m3 and pending cleared
- ✓ duplicate report is a no-op
